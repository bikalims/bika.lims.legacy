...
class ReflexRuleField(RecordsField):
    ...
    def _check_set_values(self, instance, dic):
        ...
        if not conditions or not self._check_conditions(instance, conditions):
            return False
        # Checking the actions
        actions = dic.get('actions', [])
        if not actions or not self._check_actions(instance, actions):
            return False
        return True


    def _check_conditions(self, instance, conditions):
        uc = getToolByName(instance, 'uid_catalog')
    ...
    def _check_actions(self, instance, actions):
        uc = getToolByName(instance, 'uid_catalog')
