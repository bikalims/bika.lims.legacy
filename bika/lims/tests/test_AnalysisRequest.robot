# BIKA ROBOT TEST STYLEGUIDE
#
# Leave 2 blank lines between tests
# Leave 4 spaces between Keywords
# Align Keywords 4 spaces from the longest Shortcut
#
# Run Robot Tests:
# bin/test -m bika.lims --test-file-pattern test_robot
#
*** Settings ***
...
Analysis request with sequence_start in Bika Setup
    ...
    Create Simple AR       # Create exactly one AR (see Keyword Section)
...
Check CCContacts widget basic functionality
    ...
    Select from dropdown                CCContact-0    Rita
    sleep                               .5
    Element Should Contain              CCContact-0-listing    Rita


Check that the editable SamplePoint widget in AnalysisRequestView shows both Client and Lab items
    Enable autologin as                 LabManager
    Set up Auto print stickers          # Don't print stickers automatically
    ...
    wait until page contains element    css=#SamplePoint-0
...
Check the AR Add javascript
   # check that the Contact CC auto-fills correctly when a contact is selected
    ...
    Input text                       xpath=//input[@name='ar_count']    1
    Click Link                       Add
    SelectDate                       SamplingDate-0    1
    Select From Dropdown             SampleType-0    Water
    Select from dropdown             Contact-0    Rita
    Xpath Should Match X Times       //div[@class='reference_multi_item']    1
    Select from dropdown             Contact-0    Neil
    Select from dropdown             Priority-0    High
...
#    Enable autologin as            LabManager
#    Set autologin username         test_labmanager
#    Set up Auto print stickers     # Disable automatic Sticker Printing
...
#    Execute transition sample inside ClientARView/ManageResults
#    Select From List                //select[@field='getPreserver']    Lab Preserver 1
#    @{time} =                       Get Time        year month day hour min sec
...
Create AR in ${client_id} with contact ${contact}
    ...
    Click Link                          Add
    Wait until page contains            Request new analyses
    wait until page contains element    Contact-0
    Select from dropdown                Contact-0    ${contact}
    Select from dropdown                Template-0    Bore
    ...
    ${ar_id} =                          Get text    xpath=//dl[contains(@class, 'portalMessage')][2]/dd
...
Create Simple AR
    ...
    Wait until page contains            Request new analyses
    Select from dropdown                Contact-0    Rita
    Select from dropdown                SampleType-0    Barley
    @{time} =                           Get Time   year month day hour min sec
    Select Date                         SamplingDate-0    @{time}[2]
...
Create Secondary AR
    ...
    Wait until page contains            Request new analyses
    Select from dropdown                Contact-0    Rita
    Select from dropdown                Template-0    Bruma
    select from dropdown                Sample-0    H2O
    Set Selenium Timeout                30
    Click Button                        Save
    Wait until page contains            created
    Set Selenium Timeout                2
...
In a client context, only allow selecting samples from that client.
    ...
    Click Link                          Add
    Wait until page contains            Request new analyses
    Select from dropdown                Contact-0    Johanna
    Select from dropdown                Template-0    Bore    1
    Run keyword and expect error        *    Select from dropdown    Sample-0


Complete ar_add form with template ${template}
    Wait until page contains    Request new analyses
    @{time} =                   Get Time    year month day hour min sec
    SelectDate                  SamplingDate-0    @{time}[2]
    Select from dropdown        Contact-0    Rita
    Select from dropdown        Priority-0    High
    Select from dropdown        Template-0    ${template}
    Sleep                       5
    Click Button                Save
    Wait until page contains    created
    ${ar_id} =                  Get text    xpath=//dl[contains(@class, 'portalMessage')][2]/dd
...
Submit results with out of range tests
    ...
    ${count} =                  Get Matching XPath Count    xpath=//input[@type='text' and @field='Result']
    ${count} =                  Convert to integer    ${count}
    # XXX Exclamation Mark not displayed?
...
Add new ${service} analysis to ${ar_id}
    ...
    select checkbox             xpath=//input[@alt='${service}']
...
Define container ${container} and preservation ${preservation} from Sample Partitions
    Select From List    xpath=//select[@field='getContainer']    ${container}
    Select From List    xpath=//select[@field='getPreservation']    ${preservation}
